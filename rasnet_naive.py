...
def load_vgg16conv(net, vgg16weight_pth_path):
    net_keys =  net.state_dict().keys()
    vgg16weight_pth = torch.load(vgg16weight_pth_path, map_location=lambda storage, loc: storage)
    vgg16_keys = vgg16weight_pth.keys()

    for i in range(len(vgg16_keys)):
        key_i = vgg16_keys[i]
        net_key_i = net_keys[i]
        if 'features' not in key_i: break
        net.state_dict()[net_key_i].copy_(vgg16weight_pth[key_i].cpu()) 
    return net


def load_vgg16conv_fromcaffe(net, vgg16caffemodel_pth):
    vgg16weights = torch.load(vgg16caffemodel_pth)
    weights_keys = net.state_dict().keys()
    weights_keys = [ x for x in weights_keys if ( x.startswith('conv') and ('dsn' not in x) and ('fuse' not in x) ) ]
    for k in weights_keys:
        strings = k.split('.')
        assert(len(strings) == 3)
        strings[1] = str( int(strings[1])//2 + 1 )
        new_key = strings[0] + '_' + strings[1] + '_' + strings[2]
        net.state_dict()[k].copy_(vgg16weights[new_key])
    return net 
...
if __name__ == '__main__':
    ...
    vgg16weight_pth_path = 'vgg16-imagenet-from-caffe.pth'
    net = load_vgg16conv(net, vgg16weight_pth_path)
